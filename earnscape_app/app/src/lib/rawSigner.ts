// src/lib/rawSigner.ts - Base signer class for Starknet transactions
import { transaction, CallData, hash, stark, typedData, ETransactionVersion2, ETransactionVersion3 } from "starknet";
...
export class RawSigner {
  ...
  async signTransaction(
    ...
    if (Object.values(ETransactionVersion2).includes(details.version)) {
      ...
      Object.values(ETransactionVersion3).includes(details.version)
    ) {
      msgHash = hash.calculateInvokeTransactionHash({
        ...details,
        senderAddress: details.walletAddress,
